...
@property (nonatomic, strong) NSLayoutConstraint *rightUtilityClipRightConstraint;
@property (nonatomic, strong) NSLayoutConstraint *leftUtilityClipRightConstraint;
...
- (void)setRightMargin:(CGFloat)rightMargin{
    ...
    //右边侧滑区域右边到 整个cell右边 的约束
    _rightUtilityClipRightConstraint.constant = -rightMargin;
}

- (void)setLeftMargin:(CGFloat)leftMargin{
    _leftMargin = leftMargin;
    _leftUtilityClipRightConstraint.constant = _leftMargin;
}
...
- (void)initializer
{
    ...
    _rightMargin = 0;//default rightMargin
    ...
    for (NSUInteger i = 0; i < 2; ++i)
    {
        ...
        if (clipView == self.rightUtilityClipView) {
            [self addConstraints:@[
                                   ...
                                   _rightUtilityClipRightConstraint = [NSLayoutConstraint constraintWithItem:clipView attribute:alignmentAttribute relatedBy:NSLayoutRelationEqual toItem:self attribute:alignmentAttribute multiplier:1.0 constant:-_rightMargin],
                                   clipConstraint,
                                   ]];
        }else if (clipView == self.leftUtilityClipView){
            [self addConstraints:@[
                                   // Pin the clipping view to the appropriate outer edges of the cell.
                                   [NSLayoutConstraint constraintWithItem:clipView attribute:NSLayoutAttributeTop relatedBy:NSLayoutRelationEqual toItem:self attribute:NSLayoutAttributeTop multiplier:1.0 constant:0.0],
                                   [NSLayoutConstraint constraintWithItem:clipView attribute:NSLayoutAttributeBottom relatedBy:NSLayoutRelationEqual toItem:self attribute:NSLayoutAttributeBottom multiplier:1.0 constant:0.0],
                                   _leftUtilityClipRightConstraint = [NSLayoutConstraint constraintWithItem:clipView attribute:alignmentAttribute relatedBy:NSLayoutRelationEqual toItem:self attribute:alignmentAttribute multiplier:1.0 constant:_leftMargin],
                                   clipConstraint,
                                   ]];
        }
...
- (void)updateCellState
{
    if(layoutUpdating == NO)
    {
        ...
        CGRect frame = [self.contentView.superview convertRect:self.contentView.frame toView:self];//内容区域相对于整个cell的frame
        frame.size.width = CGRectGetWidth(self.frame);
        
        self.leftUtilityClipConstraint.constant = MAX(0, CGRectGetMinX(frame) - CGRectGetMinX(self.frame))+_leftMargin;
        //内容区域左移 最右的x坐标 - 整个cell的最右x值 表示内容区域相对于cell的右边移动了多少距离 这个距离就是右边滑出区域的宽度
        self.rightUtilityClipConstraint.constant = MIN(0, CGRectGetMaxX(frame) - CGRectGetMaxX(self.frame))-_rightMargin;
        
        if (self.isEditing) {
            self.leftUtilityClipConstraint.constant = _leftMargin;
            self.cellScrollView.contentOffset = CGPointMake([self leftUtilityButtonsWidth]+_leftMargin, 0);
            _cellState = kCellStateCenter;
        }
        
        self.leftUtilityClipView.hidden = (self.leftUtilityClipConstraint.constant == _leftMargin);
        self.rightUtilityClipView.hidden = (self.rightUtilityClipConstraint.constant == -_rightMargin);
